...
namespace PlanX.Admin.Controllers
{
    ...
    public abstract partial class BaseNopController : Controller
    {
        ...
        {
            string dataKey = string.Format("PlanX.notifications.{0}", type);
        ...
        protected void SaveSelectedTabIndex(int? index = null, bool persistForTheNextRequest = true)
        {
            ...
            //"GetSelectedTabIndex" method of \PlanX.Web.Framework\ViewEngines\Razor\WebViewPage.cs
            if (!index.HasValue)
            {
                int tmp = 0;
            ...
            if (index.HasValue)
            {
                string dataKey = "PlanX.selected-tab-index";
                if (persistForTheNextRequest)
                {
